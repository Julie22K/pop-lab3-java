...
public class Main {
    ...
    private void starter(int storageSize, int itemNumbers,int num_consumers,int num_producers) {
        ...
        for (int i=0;i<num_consumers;i++){
            int itemNumbersForOneConsumer=(i != num_consumers - 1) ? itemNumbers / num_consumers : itemNumbers - (i * (itemNumbers / num_consumers));
            new Consumer(itemNumbersForOneConsumer, manager,"Consumer#" + i);
        }
        for (int i=0;i<num_producers;i++){
            int itemNumbersForOneProducer=(i != num_producers - 1) ? itemNumbers / num_producers : itemNumbers - (i * (itemNumbers / num_producers));
